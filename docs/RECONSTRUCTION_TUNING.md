...
| `tile_fov_mm` | `null` | Set to the tile field-of-view (e.g. `0.875`) only when re-using older `shifts_xy.csv` files that contain mosaic-expansion artifacts. New shift files generated by `linum-estimate-xy-shift-from-metadata` no longer need this. |
