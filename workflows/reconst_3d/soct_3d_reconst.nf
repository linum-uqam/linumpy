...
workflow {
    ...
    def selectedIds = mosaicFiles
        .collect { f -> Helpers.extractSliceId(f) }
        .findAll { sid ->
            if (debugSlices != null) {
                return debugSlices.contains(sid)
            }
            if (slicesToUse != null) {
                return slicesToUse.contains(sid)
            }
            return true
        }
    ...
    inputSlices = channel.fromList(mosaicFiles)
        ...
        .filter { slice_id, _files ->
            if (debugSlices != null) {
                ...
                if (!included) {
                    log.debug("Skipping slice ${slice_id} (not in debug_slices)")
                }
                return included
            }
            if (slicesToUse != null) {
    ...
    if (runComparison) {
        ...
        comparison_input = motor_stitch_with_id.combine(refined_stitch_with_id, by: 0)
