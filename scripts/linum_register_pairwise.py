...
def find_best_z(fixed_vol, moving_slice, expected_z, search_range, mask=None):
    """
    ...
    # Clamp expected_z to valid range first
    ...
    moving_roi = normalize(moving_slice[roi])
    if mask is not None:
    ...
    for z in range(z_min, z_max + 1):
        ...
        fixed_roi = normalize(fixed_slice[roi])
        if mask is not None:
...
def register_refinement(fixed, moving, enable_rotation=True,
                        max_rotation_deg=5.0, max_translation_px=20.0,
                        fixed_mask=None, moving_mask=None):
    """
    Compute small rotation and translation refinement.
    ...
    # Multi-resolution for robustness
    ...
    try:
        ...
        if mag > max_translation_px:
            logger.warning(f"Translation {mag:.1f}px exceeds limit {max_translation_px}px, clamping")
            scale = max_translation_px / mag
            tx, ty = tx * scale, ty * scale

        if abs(angle_deg) > max_rotation_deg:
            logger.warning(f"Rotation {angle_deg:.2f}° exceeds limit {max_rotation_deg}°, clamping")
            angle_deg = np.clip(angle_deg, -max_rotation_deg, max_rotation_deg)
...
def create_transform(tx, ty, angle_deg, center):
    """Create 3D SimpleITK transform from 2D parameters."""
    # Create 3D Euler transform (only rotates around Z axis)
