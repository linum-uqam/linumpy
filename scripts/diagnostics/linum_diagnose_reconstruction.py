...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    # Analysis selection
    p.add_argument("--skip_rotation", action="store_true", help="Skip rotation drift analysis")
    p.add_argument("--skip_shifts", action="store_true", help="Skip shifts analysis")
    p.add_argument("--skip_edge", action="store_true", help="Skip edge alignment analysis")
    ...
    p.add_argument("--slice_range", type=str, default=None, help='Analyze only these slices (e.g., "10-20" or "5,10,15")')
...
def analyze_shifts(pipeline_dir: Path, output_dir: Path, resolution: float = 10.0, slice_ids: Any = None) -> dict | None:
    ...
    if not shifts_path.exists():
        logger.warning("No shifts_xy.csv found")
        return None
...
def main() -> None:
    ...
    if not args.skip_rotation:
        ...
        results["rotation"] = analyze_rotation_drift(pipeline_dir, output_dir, args.rotation_threshold, slice_ids)

    if not args.skip_shifts:
        logger.info("Running shifts analysis...")
        results["shifts"] = analyze_shifts(pipeline_dir, output_dir, args.resolution, slice_ids)
