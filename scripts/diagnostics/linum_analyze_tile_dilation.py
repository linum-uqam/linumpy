...
def estimate_scale_factors(expected: Any, actual: Any) -> dict:
    ...
    # Y direction (rows)
    slope_y, intercept_y, r_y, _p_y, _se_y, _intercept_se_y = stats.linregress(expected[:, 0], actual[:, 0])

    # X direction (cols)
    slope_x, intercept_x, r_x, _p_x, _se_x, _intercept_se_x = stats.linregress(expected[:, 1], actual[:, 1])
