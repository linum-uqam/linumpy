...
def align_sitk_2d(prev_mosaic, img, max_allowed_overlap, method):
    best_offset = 0
    min_error = np.inf
    best_warp = np.zeros(img.shape)
    for i in range(1, max_allowed_overlap + 1):
        warped_img, error = register_consecutive_3d_mosaics(prev_mosaic[-i, :, :], img, method)
        if error < min_error:
            min_error = error
            best_offset = i
