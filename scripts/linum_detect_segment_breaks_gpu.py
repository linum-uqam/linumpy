...
GPU-accelerated version using CuPy for AIP normalisation.
...
Note: the SimpleITK registration optimizer always runs on CPU even in this GPU
version.  The GPU path accelerates only AIP normalisation.
...
Usage
-----
    linum_detect_segment_breaks_gpu.py <slices_dir> <out_directory> \\
...
def _build_arg_parser():
    ...
    p.add_argument('--use_gpu', default=True,
                   ...
                   help='Use GPU for AIP normalisation when available. '
                        'Note: the SimpleITK registration optimizer always '
                        'runs on CPU. [%(default)s]')
...
def _normalize(image: np.ndarray, use_gpu: bool = False) -> np.ndarray:
    ...
    if use_gpu and GPU_AVAILABLE:
        ...
        nonzero = img_gpu[valid]
        pmin = float(cp.percentile(nonzero, 5))
        pmax = float(cp.percentile(nonzero, 95))
        if pmax <= pmin:
            return to_cpu(cp.zeros_like(img_gpu))
        result = cp.clip((img_gpu - pmin) / (pmax - pmin), 0.0, 1.0)
...
def register_full_range(
    ...
    """
    fixed_n = _normalize(fixed_aip, use_gpu=use_gpu)
    moving_n = _normalize(moving_aip, use_gpu=use_gpu)
...
def main():
    ...
    logger.info(f"GPU normalisation: {'enabled' if use_gpu else 'disabled'}")
