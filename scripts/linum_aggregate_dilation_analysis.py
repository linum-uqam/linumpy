...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("output_directory", help="Output directory for aggregated results")

    p.add_argument(
        "--pattern",
        type=str,
        default="*/dilation_analysis/dilation_analysis.json",
        help="Glob pattern to find JSON files [%(default)s]",
    )
    p.add_argument(
        "--target_scale", type=float, default=1.0, help="Target scale factor (default 1.0 = motor positions are correct)"
    )
...
def generate_plots(results: Any, output_dir: str | Path) -> Path:
    ...
    ax3.axhline(y=np.mean(anisotropy), color="red", linestyle="--", label=f"Mean ({np.mean(anisotropy):.4f})")
    ...
    ax4.axvline(x=np.median(scale_y), color="blue", linestyle=":", label=f"Median Y ({np.median(scale_y):.4f})")
    ax4.axvline(x=np.median(scale_x), color="red", linestyle=":", label=f"Median X ({np.median(scale_x):.4f})")
