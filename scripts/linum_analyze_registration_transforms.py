...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--rotation_threshold", type=float, default=2.0, help="Flag rotations above this threshold (degrees) [%(default)s]"
    )
    p.add_argument(
    ...
    p.add_argument("--include_tfm", action="store_true", help="Also parse transform.tfm files for rotation (if JSON missing)")
...
def analyze_rotation_drift(df: Any, rotation_threshold: float = 2.0, cumulative_threshold: float = 5.0) -> Any:
    ...
    if len(large_rotations) > 0:
        analysis["issues"].append(f"Found {len(large_rotations)} slices with rotation > {rotation_threshold}°")
    ...
    if abs(analysis["cumulative_drift"]["total"]) > cumulative_threshold:
        analysis["issues"].append(f"High cumulative rotation drift: {analysis['cumulative_drift']['total']:.2f}°")

    # Check for systematic rotation bias
    if abs(analysis["rotation_stats"]["mean"]) > 0.5:
        analysis["issues"].append(f"Systematic rotation bias detected: mean={analysis['rotation_stats']['mean']:.3f}°")
...
def generate_plots(df: Any, output_dir: Path) -> Path:
    ...
    if "rotation" in df.columns and "translation_magnitude" in df.columns:
        valid = df.dropna(subset=["rotation", "translation_magnitude"])
        ax3.scatter(valid["rotation"].abs(), valid["translation_magnitude"], alpha=0.6, c=valid["slice_id"], cmap="viridis")
    ...
    if "translation_x" in df.columns and "translation_y" in df.columns:
        ...
        ax4.scatter(valid["translation_x"], valid["translation_y"], alpha=0.6, c=valid["slice_id"], cmap="viridis")
