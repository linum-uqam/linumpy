...
def _build_arg_parser():
    ...
    p.add_argument("out_volume", help="Output stacked volume (.ome.zarr)")
    ...
    p.add_argument(
        "--center_drift", action="store_true", default=True, help="Center cumulative drift around middle slice [%(default)s]"
    )
    p.add_argument("--no_center_drift", action="store_false", dest="center_drift", help="Do not center drift")
    p.add_argument(
        "--slice_pattern",
        type=str,
        default=r"slice_z(\d+)",
    ...
    p.add_argument("--preview", type=str, default=None, help="Output path for preview image (.png)")
    p.add_argument("--max_slices", type=int, default=None, help="Maximum number of slices to stack (for testing)")
