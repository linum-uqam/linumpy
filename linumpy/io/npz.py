...
def write_numpy(npz_path: str, *, data: Optional[Any] = None, metadata: Optional[Any] = None):
    ...
    ----------
    npz_path:
        The path to the .npz file to write to.
    data:
        The data to write to the file. Defaults to None.
    metadata:
        The metadata to write to the file. Defaults to None.
    """
    np.savez_compressed(
        npz_path,
        data=np.array([data]),
...
def read_numpy(npz_path: str) -> Tuple[Any, Any]:
    ...
    ----------
    npz_path:
        The path to the .npz file to read from.

    Returns
    -------
    Tuple[Any, Any]:
        A tuple containing the data and metadata read from the file.
    """
...
def read_numpy_data(npz_path: str) -> Tuple[Any, type]:
    ...
    -------
    Tuple[Any, type]:
        The data and its type read from the file.
    """
    npz = np.load(npz_path, allow_pickle=True)
...
def read_numpy_metadata(npz_path: str) -> Tuple[Any, type]:
    ...
    -------
    Tuple[Any, type]:
        The metadata and its type read from the file.
    """
    npz = np.load(npz_path, allow_pickle=True)
...
def _example_two():
    """Usage example with a custom class"""
    class Person:
