...
def analyze_metadata(data_dir: Path, axial_res_um: float, n_calibration_slices: int = 1) -> dict:
    ...
    if tissue_dirs:
        ...
        if meta_path.exists():
            try:
                ...
                result["error"] = f"Malformed JSON in {meta_path}: {exc}"
                return result
    ...
    if state_path.exists():
        try:
            ...
            result["error"] = f"Malformed JSON in {state_path}: {exc}"
            return result
...
def build_config_snippet(shift_stats: dict, acq: dict, args: argparse.Namespace) -> str:
    ...
    if acq.get("ok") and acq.get("overlap_fraction") is not None:
        ...
        initial_overlap_src = overlap_src
    else:
    ...
    lines = [
        ...
        "  // ↑ 3D pipeline (reconst_3d/nextflow.config)",
        f"initial_overlap = {initial_overlap}  {initial_overlap_src}",
        "  // ↑ 2.5D pipeline (reconst_2.5d/soct_2.5d_reconst.nf); mirrors stitch_overlap_fraction",
        "",
        "// ── Slice registration ────────────────────────────────────────",
