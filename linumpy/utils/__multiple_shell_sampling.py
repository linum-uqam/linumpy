...
# Adapted from the scilpy/gradientsampling code on github https://github.com/scilus/scilpy
...
def cost(bvecs, S, Ks, weights):
    ...
    for s1 in range(S):
        for s2 in range(S):
            weight_matrix[indices[s1]:indices[s1 + 1], indices[s2]:indices[s2 + 1]] = weights[s1, s2]
